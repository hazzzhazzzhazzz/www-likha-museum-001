<?php
    ...
    class CustomerUserManagement {
        ...
        public function createRegistration($firstName, $middleName, $lastName, $email, $contactNumber, $birthDate, $customerTypeID) {
            ...
            $query_tblCustomers = "INSERT INTO tbl_customers (CustomerType_ID, FirstName, MiddleName, LastName, Email, ContactNumber, BirthDate, createdAt, updatedAt) 
            VALUES (:customerTypeID, :firstName, :middleName, :lastName, :email, :contactNumber, :birthDate, :createdAt, :updatedAt)";

            $response_tblCustomers = $this -> conn -> prepare($query_tblCustomers);

            $response_tblCustomers -> bindParam(':customerTypeID', $customerTypeID);
            $response_tblCustomers -> bindParam(':firstName', $firstName);
            $response_tblCustomers -> bindParam(':middleName', $middleName);
            $response_tblCustomers -> bindParam(':lastName', $lastName);
            $response_tblCustomers -> bindParam(':email', $email);
            $response_tblCustomers -> bindParam(':contactNumber', $contactNumber);
            $response_tblCustomers -> bindParam(':birthDate', $birthDate);
            $response_tblCustomers -> bindParam(':createdAt', $dateNow);
            $response_tblCustomers -> bindParam(':updatedAt', $dateNow);

            $response_tblCustomers -> execute();

            $customerID = $this -> conn -> lastInsertId();


            
            $query_tblAdminDetails = "INSERT INTO tbl_admindetails (Customer_ID, PasswordHash, createdAt, updatedAt) 
            VALUES (:customerID, :passwordHash, :createdAt, :updatedAt)";

            $response_tblAdminDetails = $this -> conn -> prepare($query_tblAdminDetails);

            $response_tblAdminDetails -> bindParam(':customerID', $customerID);
